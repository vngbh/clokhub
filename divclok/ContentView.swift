...
struct ContentView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            VStack {
                ...
                // Pie chart + summary
                let totalTime = accumulatedTimes.values.reduce(0, +) + currentTime.timeIntervalSince(currentStartTime)
                let percents = (0..<3).map { i -> Double in
                    let t = accumulatedTimes[i, default: 0] + (selectedIndex == i ? currentTime.timeIntervalSince(currentStartTime) : 0)
                ...
                HStack(alignment: .center, spacing: 34) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ForEach(0..<3, id: \.self) { i in
                            let tInterval = accumulatedTimes[i, default: 0] + (selectedIndex == i ? currentTime.timeIntervalSince(currentStartTime) : 0)
                ...
                ZStack {
                    ...
                    ZStack {
                        ForEach(0..<3, id: \.self) { i in
                            ...
                            PieSlice(startAngle: .degrees(start), endAngle: .degrees(start + sliceAngle))
                                ...
                                .overlay(
                                    PieSlice(startAngle: .degrees(start), endAngle: .degrees(start + sliceAngle))
                                        .stroke(pastelColors[i],
                                                style: StrokeStyle(lineWidth: cornerRadius * 12, lineCap: .round, lineJoin: .round))
                                )
                    ...
                    let segmentTime = accumulatedTimes[selectedIndex, default: 0] + currentTime.timeIntervalSince(currentStartTime)
    ...
    private func saveCurrentState() {
        ...
        let encodedTimes = Dictionary(uniqueKeysWithValues: accumulatedTimes.map { (key, val) in
            (String(key), val)
        })
    ...
    private func loadSavedState() {
        if let saved = UserDefaults.standard.dictionary(forKey: "accumulatedTimes") as? [String: TimeInterval] {
        ...
        let sliceAngle = 360.0 / 3
        let midAngle = (Double(selectedIndex) * sliceAngle + sliceAngle / 2).truncatingRemainder(dividingBy: 360)
...
struct GrayTrailArc: View {
    ...
    var body: some View {
        TrailShape(startAngle: .degrees(progress * 360 - 45),
                   sweep: .degrees(90))
            .stroke(
    ...
    var body: some View {
        ZStack {
            ForEach(percentages.indices, id: \.self) { i in
                ...
                PieSlice(startAngle: .degrees(start),
